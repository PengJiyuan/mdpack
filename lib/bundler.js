...
function getDefaults() {
  return {
    ...
    template: path.resolve(__dirname, 'template/wrapper.ejs'),
  };
}
...
function bundler(mdpackConfig = {
  ...
  mdpackConfig.format.forEach((f) => {
    if (f === 'html') {
      ...
      //   html = html.replace(new RegExp(`<% ${rc} %>`, 'g'), resources[rc]);
      // });
      // html = html.replace('<% content %>', pureHtml);
      resources.pureHtml = pureHtml;
      html = ejs.render(html, resources);
